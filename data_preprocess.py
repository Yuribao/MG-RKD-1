...
def normalize(mx):#对稀疏矩阵（邻接矩阵）进行归一化处理
...
def normalize_adj(adj):#添加自环后归一化处理
    adj = normalize(adj + sp.eye(adj.shape[0]))
    return adj


def eliminate_self_loops_adj(A):#移除邻接矩阵中的自环
...
def largest_connected_components(sparse_graph, n_components=1):#从稀疏图中提取最大连通子图
...
def create_subgraph(#通过保留或删除特定节点来创建子图
...
def binarize_labels(labels, sparse_output=False, return_classes=False):#将类别标签转换为二值标签矩阵
...
def remove_underrepresented_classes(#删除那些节点数过少的类别，以避免训练时出现问题
